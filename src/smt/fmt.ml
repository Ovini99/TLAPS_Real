...
(* Util.eprintf "%a <<< %s" (print_prop ()) e prop.rep ; *)
  ...
  | "real2u"   -> Apply (Opaque "real2u" |> noprops, [e]) |> noprops
  | "str2u"   -> Apply (Opaque "str2u" |> noprops, [e]) |> noprops
  | "fmtasint" -> assign e fmtasint ()
  | "fmtasreal" -> assign e fmtasreal ()
  | _ -> e
...
(* Util.eprintf "lift %a\t%a" Typ_e.ppt (Typ_e.empty,req) (Typ_e.pp_print_expr (scx, Ctx.dot)) e ; *)
  ...
  | Apply ({core = Internal op} as o, es) ->
      ...
      | B.Mem, [x ; {core = Internal B.Int}] when Typ_t.is_int scx x ->
          Internal B.TRUE %% []
      | B.Mem, [x ; {core = Internal B.Real}] when Typ_t.is_real scx x ->
          Internal B.TRUE %% []
...
(* Util.eprintf "     %a = %a" Typ_e.ppt (Typ_e.empty,t1) Typ_e.ppt (Typ_e.empty,t2); *)
          ...
          | Int,  Int  -> 
            apply op (lift Int scx e1) (lift Int scx e2)
          | Real,  Real  -> 
            apply op (lift Real scx e1) (lift Real scx e2)
      ...
      | B.Gt,     [e1 ; e2] ->
          ...
          | Real, Real -> apps (o <<< "fmtasreal") [lift Real scx e1 ; lift Real scx e2]
      ...
      | B.Exp,       [e1 ; e2]
          ...
          | _         -> 
            begin match req, Typ_t.is_real scx e1 && Typ_t.is_real scx e2 with
            | Top, true -> apps (o <<< "fmtasreal") [lift Real scx e1 ; lift Real scx e2] <<< "real2u"
            | _,   true -> apps (o <<< "fmtasreal") [lift Real scx e1 ; lift Real scx e2]
            | _         -> 
      ...
      | B.Uminus, [ex] ->
          ...
          | _         ->
            begin match req, Typ_t.is_real scx ex with
            | Top, true -> apps (o <<< "fmtasreal") [lift Real scx ex] <<< "real2u"
            | _, true   -> apps (o <<< "fmtasreal") [lift Real scx ex]
  ...
  | Num (m, "") ->
      if req = Int then e else e <<< "int2u"
  | Num (n, m) ->
      if req = Real then e else e <<< "real2u"
  (* | Internal B.Infinity ->
...
(* Printf.eprintf "\te2 :: %s\n" (TLAtype.to_string t2) ; *)
      ...
      | Real, Real, Real   -> Real, Real, None
      | Top, Int, Int   -> Top, Top, None
      | Top, Real, Real   -> Top, Top, None
      | Int, Int, Top   -> Top, Top, None
      | Real, Real, Top   -> Top, Top, None
      | Top, Int, Top   -> Top, Top, None
      | Top, Real, Top   -> Top, Top, None
      | Int, Top, Int   -> Top, Top, None
      | Real, Top, Real   -> Top, Top, None
      | Top, Top, Int   -> Top, Top, None
      | Top, Top, Real   -> Top, Top, None
...
(* Util.eprintf "lift %a-%a\t%a" Typ_e.ppt (Typ_e.empty,t) Typ_e.ppt (Typ_e.empty,req) (Typ_e.pp_print_expr (Deque.empty, Ctx.dot)) e ; *)
  ...
  | Real, Top  -> e <<< "fmtasreal" <<< "real2u"
  | Str, Top  -> e <<< "str2u"
  | Int, Int -> e <<< "fmtasint"
  | Real, Real -> e <<< "fmtasreal"
  (* | Str, Str -> e <<< "fmtasstr" *)
  | _          -> e
  end
and lift_bs scx bs =
