...
let is_Real scx e = T.is_real (snd scx) e
...
(* Util.eprintf "rw: %a" (Expr.Fmt.pp_print_expr (snd scx, Ctx.dot)) e ; *)
    ...
    (* let reals = Internal B.Real |> mk in *)
    let leq x y = apply2 B.Lteq x y in
    let mk_num n = Num (string_of_int n,"") |> mk in
    (* let mk_real n m = Num (string_of_int n,string_of_int m) |> mk in *)
    ...
    let rw_mem x y =
        ...
        begin match op2 with
          ...
          | B.Range ->
            begin match [e1; e2] with
              | [{core=Num (n,"")};{core=Num (n1,"")}] ->
                lAnd [mem x (Internal B.Int |> mk) ; leq e1 x ; leq x e2] |> self#expr scx
              | [{core=Num (n,"")};_] ->
                lAnd [mem x (Internal B.Int |> mk) ; leq e1 x ; leq x e2] |> self#expr scx
              | [_;{core=Num (n1,"")}] ->
                lAnd [mem x (Internal B.Int |> mk) ; leq e1 x ; leq x e2] |> self#expr scx
              | [{core=Num (n,m)};{core=Num (n1,m1)}] ->
                lAnd [mem x (Internal B.Real |> mk) ; leq e1 x ; leq x e2] |> self#expr scx
              | [_;{core=Num (n1,m1)}] ->
                lAnd [mem x (Internal B.Real |> mk) ; leq e1 x ; leq x e2] |> self#expr scx
              | [{core=Num (n,m)};_] ->
                lAnd [mem x (Internal B.Real |> mk) ; leq e1 x ; leq x e2] |> self#expr scx
      ...
      | _, Internal B.Real when !Smt.mode != Smt.Fof && !Smt.mode != Smt.Spass && T.is_real (snd scx) x ->
        tla_true
...
*)
      ...
      *)
      ...
      | Apply ({core = Internal B.Range}, [a ; b]), Apply ({core = Internal B.Range}, [a' ; c])
        when is_Real scx a && is_Real scx b && is_Real scx a' && is_Real scx c && Expr.Eq.expr a a' ->
        lOr [eq b c ; lAnd [lt b a ; lt c a]] |> self#expr scx
          ...
          lAnd [
            ...
            forall ~dom:(range m n) ~typ:(Some T.Int)
                (eq (fcnapp (sh1 x) (minus (plus one ix1) (sh1 m)))
                   (fcnapp (sh1 s) ix1));
            forall ~dom:(range m n) ~typ:(Some T.Real)
                (eq (fcnapp (sh1 x) (minus (plus one ix1) (sh1 m)))
                   (fcnapp (sh1 s) ix1))
          ] |> self#expr scx
