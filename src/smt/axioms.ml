...
type smtsort = U | SBool | SInt | SReal | SStr | SField | SOp
  ...
  quant = begin fun ff q pat vs ppex ex ->
    ...
    fprintf ff "@[<hov2>(%s@ (@[<v>%s@])@ %s%a%s%a%s)@]" q
      (vs |> List.map (fun (v,t) -> sprintf "@[(%s %s)@]" v
              (match t with SBool -> "Bool" | SInt -> "Int" | SReal -> "Real" | SStr -> "str" | SField -> "tla_Field" | _ -> "u"))
          |> String.concat " ")
...
let build_tla_ops (smap:t_map) std_set (* tuples record_ids *) =
  ...
  let real2u x     = app "real2u" [x] in    (** Fof and Spass will print [x] and ignore the lifting *)
  ...
  let tla_ops_fof = [
      ...
      "div",       [SReal;SReal], SReal,  [ forall [x,SReal ; y,SReal ; z,SReal]
            (implies (lt "0" y)
            (equiv (eq (app "div" [x ; y]) z)
                          (lAnd [ leq (app2_op B.Times y z) x ;
                                      leq x (app2_op B.Times y (app2_op B.Plus z "1"))]))) ], [] ;
      ...
      "mod",       [SReal;SReal], SReal,  [ forall [x,SReal ; y,SReal]
            (implies (lt "0" y)
            (eq (app "mod" [x ; y])
            (app2_op B.Minus x (app2_op B.Times (app "div" [x ; y]) y)) )) ], ["div"] ;
  ] in
  ...
  let tla_ops_spass = [
      ...
      "real2u",     [SReal], U, [ forall [x,SReal ; y,SReal] (implies (eq (real2u x) (real2u y)) (eq x y)) ], [] ;
      "str2u",     [SStr], U, [ forall [x,SStr ; y,SStr] (implies (eq (str2u x) (str2u y)) (eq x y)) ], [] ;
  ] @ tla_ops_fof
  in

  let lift_arith sop top =
    forall [m,(if List.mem "real2u" std_set then SReal else SInt) ; n,(if List.mem "real2u" std_set then SReal else SInt)] (pattern
      (eq (app2 sop ((if List.mem "real2u" std_set then real2u else int2u) m) ((if List.mem "real2u" std_set then real2u else int2u) n))
          ((if List.mem "real2u" std_set then real2u else int2u) (app2_op top m n)))
      [app2 sop ((if List.mem "real2u" std_set then real2u else int2u) m) ((if List.mem "real2u" std_set then real2u else int2u) n)])
  in
  ...
  let lift_arith_predicate sop top =
    forall [m,(if List.mem "real2u" std_set then SReal else SInt) ; n,(if List.mem "real2u" std_set then SReal else SInt)] (pattern
      (equiv (app2 sop ((if List.mem "real2u" std_set then real2u else int2u) m) ((if List.mem "real2u" std_set then real2u else int2u) n))
             (app2_op top m n))
      [app2 sop ((if List.mem "real2u" std_set then real2u else int2u) m) ((if List.mem "real2u" std_set then real2u else int2u) n)])
  in
  let tla_ops_smtlib = [
      ...
      "real2u",     [SReal], U, [ forall [x,SReal] (pattern (eq (app "u2real" [real2u x]) x) [ real2u x ]) ], [ "u2real"] ;
      "u2int",     [U], SInt, [], [] ;
      "u2real",     [U], SReal, [], [] ;
  ...
  let all_std_ops =
    ...
    m_tla B.Plus,  [U;U], U, [ lift_arith (m_tla B.Plus)  B.Plus ], [if List.mem "real2u" std_set then "real2u" else "int2u"] ;
    m_tla B.Minus, [U;U], U, [ lift_arith (m_tla B.Minus) B.Minus ], [if List.mem "real2u" std_set then "real2u" else "int2u"] ;
    m_tla B.Times, [U;U], U, [ lift_arith (m_tla B.Times) B.Times ], [if List.mem "real2u" std_set then "real2u" else "int2u"] ;
    (* m_tla B.Exp,   [U;U], U,     [ lift_arith (m_tla B.Exp)   B.Exp ], ["int2u"] ; *)
    (* m_tla B.Exp,   [U;U], U,     [ ], [ ] ; *)
    m_tla B.Exp,   [U;U], U,     [ lift_arith_exp (m_tla B.Exp)   B.Exp ], [if List.mem "real2u" std_set then "real2u" else "int2u" ; smap.op B.Exp] ;
    m_tla B.Quotient, [U;U], U,  [ lift_arith_positive (m_tla B.Quotient)   B.Quotient ], [if List.mem "real2u" std_set then "real2u" else "int2u" ; smap.op B.Quotient] ;
    m_tla B.Remainder,[U;U], U,  [ lift_arith_positive (m_tla B.Remainder)  B.Remainder ], [if List.mem "real2u" std_set then "real2u" else "int2u" ; smap.op B.Remainder] ;
    m_tla B.Lt,    [U;U], SBool, [ lift_arith_predicate (m_tla B.Lt)   B.Lt ], [if List.mem "real2u" std_set then "real2u" else "int2u"] ;
    m_tla B.Lteq,  [U;U], SBool, [ lift_arith_predicate (m_tla B.Lteq) B.Lteq ], [if List.mem "real2u" std_set then "real2u" else "int2u"] ;
    m_tla B.Range, [U;U], U,  [
      forall [m,(if List.mem "real2u" std_set then SReal else SInt) ; n,(if List.mem "real2u" std_set then SReal else SInt) ; z,U ] (pattern_z3
      (equiv (mem z (app2 (m_tla B.Range) ((if List.mem "real2u" std_set then real2u else int2u) m) ((if List.mem "real2u" std_set then real2u else int2u) n)))
      (exists [x,(if List.mem "real2u" std_set then SReal else SInt)] (lAnd [eq z ((if List.mem "real2u" std_set then real2u else int2u) x) ; leq m x ; leq x n])))
      [ mem z (app2 (m_tla B.Range) ((if List.mem "real2u" std_set then real2u else int2u) m) ((if List.mem "real2u" std_set then real2u else int2u) n)) ])
      ;
      (** added sm 2019-02-20, fixed sm 2019-12-16:
          Range is injective when intervals are non-empty. *)
      forall [m,(if List.mem "real2u" std_set then SReal else SInt) ; n,(if List.mem "real2u" std_set then SReal else SInt) ; x,(if List.mem "real2u" std_set then SReal else SInt) ; y,(if List.mem "real2u" std_set then SReal else SInt) ]
        (implies
           (lAnd [ lOr [ leq m n ;
                         leq x y ] ;
                   (eq (app2 (m_tla B.Range) ((if List.mem "real2u" std_set then real2u else int2u) m) ((if List.mem "real2u" std_set then real2u else int2u) n))
                       (app2 (m_tla B.Range) ((if List.mem "real2u" std_set then real2u else int2u) x) ((if List.mem "real2u" std_set then real2u else int2u) y))) ])
           (lAnd [eq m x ; eq n y]))
      ;
      (** added sm 2019-02-22: Range is empty iff the upper bound is
          greater than the lower bound. *)
      forall [m,(if List.mem "real2u" std_set then SReal else SInt) ; n,(if List.mem "real2u" std_set then SReal else SInt) ]
        (equiv
           (eq (app2 (m_tla B.Range) ((if List.mem "real2u" std_set then real2u else int2u) m) ((if List.mem "real2u" std_set then real2u else int2u) n)) "tla__emptyset")
           (lt n m))
      ], [m_tla B.Mem ; if List.mem "real2u" std_set then "real2u" else "int2u"  ; "tla__emptyset"] ;

    m_tla B.Uminus,[U], U,    [ forall [n,(if List.mem "real2u" std_set then SReal else SInt)] (pattern
      (eq (app1 (m_tla B.Uminus) ((if List.mem "real2u" std_set then real2u else int2u) n))
          ((if List.mem "real2u" std_set then real2u else int2u) (m_uminus smap n)))
      [ app1 (m_tla B.Uminus) ((if List.mem "real2u" std_set then real2u else int2u) n) ]) ], [if List.mem "real2u" std_set then "real2u" else "int2u" ] ;
    "boolify", [U], SBool, [], [] ;
  ...
  let rec fixp_add_deps std_set =
    let ff oops =
      let deps = List.fold_left
        (fun r (o,_,_,_,deps) -> 
        (* printf "%s %b \n" o (List.mem o oops); *)
        if List.mem o oops then (deps @ r) else r)
        [] all_std_ops
      in
  ...
  (* print_endline "DEPENDENCIES ADDED"; *)
  let std = List.filter (fun (id,_,_,_,_) -> 
      (* printf "%s %b \n" id (List.mem id std_set); *)
    List.mem id std_set) all_std_ops in
