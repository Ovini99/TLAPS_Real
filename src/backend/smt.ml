...
(* Global variable used as indication if theorems contain reals by checking if real2u was added in collect_data function *)
let real2u_added = ref false
    ...
    | Internal b ->
        ...
        *)
        ...
        | B.Mem, [e ; ({core = Internal B.Int})]
            ...
            atomic (m.op B.TRUE)
        | B.Mem, [e ; ({core = Internal B.Real})]
                when T.is_real (fst ecx) e ->
            atomic (m.op B.TRUE)
        | B.Mem, [e ; f] ->
            nonatomic (Axioms.m_tla b) args
        | B.Nat, []
        ...
        | (B.Plus | B.Minus | B.Times | B.Exp), [e; f] ->
            if ((T.is_real (fst ecx) e) && T.is_real (fst ecx) f) then arith (Smt.fmt_as_real op) b args
            else arith (Smt.fmt_as_int op) b args
        | (B.Quotient | B.Remainder), [e ; f] ->
            arith (Smt.fmt_as_int op) b args
        | B.Uminus, [ex] ->
            if ((Smt.fmt_as_real op) || Smt.fmt_as_int op) then
                m.uminus ff (to_fol_expr ecx) ex
                (* fprintf ff "(- %a)" (to_fol_expr ecx) ex *)
            else                
                nonatomic "tla__uminus" args
        | B.Ratio, [e; f] ->
            if ((T.is_real (fst ecx) e) && T.is_real (fst ecx) f) then
                arith (Smt.fmt_as_real op) b args
            else
                if ((T.is_int (fst ecx) e) && T.is_int (fst ecx) f) then
                    arith (Smt.fmt_as_int op) b args
                else
...
and fmt_expr (ecx:Ectx.t) e =
    ...
    *)
    ...
    | Num (n, m) ->
        Fu.Atm (fun ff -> fprintf ff "%s.%s" n m)
    ...
    let visitor = object (self: 'self)
        ...
        method expr scx e =
            ...
            *)
            ...
            | Num (m, n) ->
                std := SSet.add "real2u" !std;
                real2u_added := true;
                nums := SSet.add (m^"."^n) !nums
            | Tuple [] ->
            ...
            | _ ->
                ...
                if (not !real2u_added) then (if (T.is_real (fst ecx) e) then (real2u_added := true; std := SSet.add "real2u" !std));
                super#expr scx e
...
let encode_smtlib ?solver:(mode="SMTLIB") ff ob =
    ...
    *)
    ...
    fprintf ff "\n";
    if (not !real2u_added) then
    (
        fprintf ff "%s" ("(set-logic " ^ !Params.smt_logic ^ ")\n");
    )
    else
    (
        fprintf ff "%s" ("(set-logic " ^ !Params.smt_logic_real ^ ")\n");
    );
