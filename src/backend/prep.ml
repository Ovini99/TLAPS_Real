...
let gen_smt_solve suffix exec desc fmt_expr meth ob org_ob f res_cont comm =
  ...
  try
    ...
    let _= print_endline "COMMAND-LINE ARGUMENT" in
    let _= print_endline cmdline in
    let in_text =
      ignore (Format.flush_str_formatter ());
    ...
    print_endline "CONTENT OF FILE SENT TO BACKEND";
    print_endline in_text;
    output_string inc in_text;
    flush inc;
    let warnings = Errors.get_warnings () in
    let contains re s =
...
let metit_solve ob org_ob f res_cont =
  gen_smt_solve ".smt2" Params.metit "METIT" (Smt.encode_smtlib ~solver:"Z3")
                (Method.METIT f) ob org_ob f res_cont ";;"
...
let prove_with ob org_ob meth save =  (* FIXME add success fuction *)
  ...
  | Method.Z33 f ->
     ...
     z3_solve ob org_ob f res_cont
  | Method.METIT f ->
     vprintf "(* ... using MetiTarski *)\n" ;
     metit_solve ob org_ob f res_cont
  | Method.Cvc33 f ->
...
let compute_meth def args usept =
  ...
  | Some "z3" ->
     ...
     Method.Z33 tmo;
  | Some "metit" ->
     let tmo = Option.default Method.default_metit_timeout !timeout in
     Method.METIT tmo;
  | Some "smt" ->
